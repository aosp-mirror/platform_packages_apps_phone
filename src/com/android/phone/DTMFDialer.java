...
/**
 * Dialer class that encapsulates the DTMF behavior.
 * A single instance of this class is created by {@link PhoneGlobals} and accessible through {@link PhoneGlobals.mDTMFDialer}.
 * This class encapsulates a FIFO queue which holds DTMF dail requests. Whenever the active forground call ends or changes the queue is cleared if not empty.
 * According to system settings a local DTMF tone is played while a tone is send to the network.
 * Local tones can also be played without sending a tones over the network.
 * This is used by {@link InCallScreen} for handling post DTMF tones initialized by phone number extensions. (for example 5034243,,2,23)
 */
public class DTMFDialer
{
    ...
    // the local tone character for playing a local tone without sending it over the network
    private Character mCurrentCharacterForLocalDialerTone = null;

    // indicate if the confirmation from TelephonyFW or the timeout for a tone is pending.
    ...
    // The DTMF start play request to be continued until a corresponding stop play request is invoked.
    private DTMFQueueEntry mDTMFQueueEntryToPlayWithNoAutoStop = null;

    // The auto stop time is over and the current tone needs to be stopped manually by a call to stopDtmfForTwelveKeyChar()
    ...
    /**
     * Interface definition for a callback to be invoked when playing a DTMF tone starts.
     */
    public interface OnStartDtmfForTwelveKeyCharListener {
        /**
         * Called when a DTMF tone has been started.
         *
         * @param c The character for which the tone was started.
         */
    ...
    private final Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            case DTMF_SEND_CNF:
                ...
                break;
            case DTMF_STOP:
                if (DBG) log("dtmf delayed stop received.");
                handleDtmfStop();
                break;
            }
        }
    ...
    public DTMFDialer()
    {
        ...
        synchronized (mLock) {
            try {
                mLocalToneGenerator = new ToneGenerator(AudioManager.STREAM_DTMF, 80);
    ...
    /**
     * Starts or queues a DTMF dial request for the current active call. The tone automatically stops after a short time.
     * If the active call ends or gets inactive before the tone could be played the request will be canceled.
     * The DTMF tone is send over the network. It is also played locally if system settings allow this.
     * @param c The DTMF character to be send over the network.
     * @return True in the case the given character is a valid DTMF twelve key character and a active call was found.
     */
    public boolean playDtmfForTwelveKeyChar(char c) {
        if (DBG) log("playDtmfForTwelveKeyChar: "+c);

        return processDtmf(c, true /*autoStop*/);
    }

    /**
     * Starts or queues a DTMF dial request for the current active call. After the tone starts the dialer will continue to play it and
     * also stops playing further queued request until a corresponding call to stopDtmfForTwelveKeyChar happens.
     * If the active call ends or gets inactive before the tone could be played, than the request will be canceled.
     * The DTMF tone is send over the network. It is also played locally if system settings allow this.
     * @param c The DTMF character to be send over the network.
     * @return True in the case the given character is a valid DTMF twelve key character and a active call was found.
     */
    public boolean startDtmfForTwelveKeyChar(char c) {
        if (DBG) log("startDtmfForTwelveKeyChar: "+c);

        return processDtmf(c, false /*autoStop*/);
    }

    /**
     * Stops playing of a DTMF tone previously started by {@link startDtmfForTwelveKeyChar}.
     * In the case the previously dial request is still in the queue and waiting to be played,
     * the requested tone will be played like a normal play request later on.
     */
    public void stopDtmfForTwelveKeyChar() {
        if (DBG) log("stopDtmfForTwelveKeyChar");

        synchronized (mLock) {
            mDTMFQueueEntryToPlayWithNoAutoStop = null;

            if (mDTMFManualStopPending)			//the current tone is waiting to be stopped manually, stop it now....
                stopToneAndTryPlayNextFromQueue();
        }
    }

    /**
     * Starts the local DTMF tone for the dialer pad. No tone is send over the network.
     * Used by InCallScreen for playing post DTMF tons included in number extensions.
     */
    ...
    /**
     * Stops the local DTMF tone for the dialer pad.
     * Used by InCallScreen for playing post DTMF tons included in number extensions.
     */
    public void stopLocalDialerToneIfNeeded() {
        if (DBG) log("stopping dialer tone.");
     ...
     */
    public void setOnStartDtmfForTwelveKeyCharListener(OnStartDtmfForTwelveKeyCharListener l) {
    ...
    private final boolean processDtmf(char c, boolean autoStop) {
        ...
        if (PhoneNumberUtils.is12Key(c)) {
            if (DBG) log("sending dtmf tone for '" + c + "'");
    ...
    /**
     * Checks if the active call has changed until a previous call to this function.
     * If the active call changed, than the request queue is reseted.
    ...
    private boolean checkActiveCallId() {
        ...
        if (activeCall != null) {
            if (activeCall.getState() == Call.State.ACTIVE || activeCall.getState() == Call.State.ALERTING) {
    ...
    private void startTone(char c, boolean autoStop) {
        ...
        // indicates that we are using automatically shortened DTMF tones for the network. (depends on the phone type)
        boolean shortTone;
    ...
    public boolean okToDialDTMFTones() {
        ...
        boolean canDial =
                ...
                && !hasRingingCall
                && (PhoneGlobals.getInstance().inCallUiState.inCallScreenMode != InCallScreenMode.MANAGE_CONFERENCE);

        if (DBG) log ("[okToDialDTMFTones] foreground state: " + fgCallState +
                ", ringing state: " + hasRingingCall +
                ", call screen mode: " + PhoneGlobals.getInstance().inCallUiState.inCallScreenMode +
                ", result: " + canDial);
    ...
    private void handleBurstDtmfConfirmation() {
        if (DBG) log("handleBurstDtmfConfirmation...");

        handleStopTone();
    }

    /**
     * Handles the dayled stop tone request for a current none Burst Dtmf tone.
     */
    private void handleDtmfStop() {
        if (DBG) log("handleDtmfStop...");
    ...
    private void handleStopTone() {
        synchronized (mLock) {
            if (mDTMFQueueEntryToPlayWithNoAutoStop == mDTMFQueueEntryInPlay) {
                //the current tone was started via a start request and need to be played until we receive a stop request
                mDTMFQueueEntryToPlayWithNoAutoStop = null;
                mDTMFManualStopPending = true;
            }
            else
                //stop the current tone and try to play the next tone from the queue
                stopToneAndTryPlayNextFromQueue();
        }
    }
        ...
        synchronized (mLock) {
            if (mDTMFQueueEntryInPlay != null){
                ...
                if (!mDTMFQueueEntryInPlay.getUseBurstDtmf())
                    //stop it in the case it is not stopped by the network itself
                    mCM.stopDtmf();

                if (mDTMFQueueEntryToPlayWithNoAutoStop == mDTMFQueueEntryInPlay)
                    //this is the stop of the current no auto stop tone -> reset it
                    mDTMFQueueEntryToPlayWithNoAutoStop = null;
            ...
            if (!mDTMFQueue.isEmpty())
                //check if we need to empty the queue because of active call changes
                checkActiveCallId();

            if (!mDTMFQueue.isEmpty()) {
                //play the next tone from the queue
                dtmfQueueEntry = mDTMFQueue.remove();
                Log.i(LOG_TAG, "The dtmf character removed from queue" + dtmfQueueEntry.getCharacter());
     ...
     * @param queueEntry The play request.
     */
    private void playDTMFQueueEntry(DTMFQueueEntry queueEntry)
    {
        ...
        synchronized (mLock) {
            if (mDTMFAutoStopPending == true) {
                ...
                if (queueEntry.getUseBurstDtmf()){
                    String dtmfStr = Character.toString(queueEntry.getCharacter());
        ...
        if (mOnStartDtmfForTwelveKeyCharListener != null)
            mOnStartDtmfForTwelveKeyCharListener.onStartDtmfForTwelveKeyChar(queueEntry.mcharacter.charValue());
    }
    ...
    private void updateLocalDTMFToneIfNeeded() {
        if (DBG) log("update local DTMF tone.");

        boolean localToneEnabled = (PhoneGlobals.getInstance().getResources().getBoolean(R.bool.allow_local_dtmf_tones)) &&
                android.provider.Settings.System.getInt(PhoneGlobals.getInstance().getContentResolver(),
                        android.provider.Settings.System.DTMF_TONE_WHEN_DIALING, 1) == 1;
        ...
        if (localToneEnabled) {
            synchronized (mLock) {
                ...
                if (mDTMFQueueEntryInPlay != null) {
                    //play the tone corresponding to the currently sent DTMF character
                ...
                if (c != null) {
                    if (mLocalToneGenerator == null) {
                        if (DBG) log("updateLocalDTMFToneIfNeeded: mLocalToneGenerator == null, tone: " + c);
                    } else {
                        if (DBG) log("starting local DTMF tone: " + c);

                        mLocalToneGenerator.startTone(mToneMap.get(c), toneDuration);
                    }
                } else {
                    if (mLocalToneGenerator == null) {
                        if (DBG) log("stopDialerTone: mLocalToneGenerator == null");
                    } else {
                        if (DBG) log("stopping dialer tone.");
