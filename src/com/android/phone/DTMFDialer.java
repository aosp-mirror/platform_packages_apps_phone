 ...
 * holds DTMF dail requests. Whenever the active forground call ends or changes
 * the queue is cleared if not empty. According to system settings a local DTMF
 * tone is played while a tone is send to the network. Local tones can also be
 * played without sending a tones over the network. This is used by
 * {@link InCallScreen} for handling post DTMF tones initialized by phone number
 * extensions. (for example 5034243,,2,23)
 */
public class DTMFDialer {
    ...
    // the local tone character for playing a local tone without sending it over
    // the network
     ...
     * {@link startDtmfForTwelveKeyChar}. In the case the previously dial
