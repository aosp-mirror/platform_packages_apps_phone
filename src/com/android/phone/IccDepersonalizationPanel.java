...
public class IccDepersonalizationPanel extends IccPanel {
    ...
    public IccDepersonalizationPanel(Context context) {
        super(context);
        mPersoSubtype = IccCardApplication.PersoSubState.PERSOSUBSTATE_SIM_NETWORK.ordinal();
    }
    ...
    private void displayStatus(int type) {
        int label = 0;
        IccCardApplication.PersoSubState persosubtype = IccCardApplication.PersoSubState.values()[mPersoSubtype];
        switch (persosubtype) {
        ...
        if (type == 0) {
            mPersoSubtypeText.setText(getContext().getString(label));
        } else {
