...
public class BluetoothHandsfree {
    ...
    //
    // Helper function to translate the SCO state to a BluetoothHeadset state
    private void broadcastCallStateChange(int scoState) {
        int state;
        if (DBG) log("broadcastCallStateChange: scoState:" + scoState);
        if (isCellularCallInProgress()) {
            state = ((scoState == SCO_CONNECTED) || (scoState == SCO_ACCEPTED)) ?
                      BluetoothHeadset.AUDIO_STATE_CONNECTED :
                      BluetoothHeadset.AUDIO_STATE_DISCONNECTED;
            broadcastAudioStateIntent(state, mHeadset.getRemoteDevice());
            ...
            broadcastVirtualCallStateIntent(state, mHeadset.getRemoteDevice());
        }
    }
    ...
    private void broadcastVirtualCallStateIntent(int state, BluetoothDevice device) {
    ...
    synchronized boolean initiateVirtualVoiceCall() {
        ...
        // 2.5 Set the AudioManager state
        // Setting the AudioManager MODE_IN_CALL shall suspend AV if music is playing over A2DP
